...
module.exports = function (app) {
  ...
  app.get('/api/v1/account/admin/:name/all-admins', authenticateToken, async (req, res)=>{
    let adminList = await adminService.getAllAdmin();
    res.json(adminList);
  });


  app.get('/api/v1/account/admin/:name/all-users', authenticateToken, async (req, res)=>{
    let userList = await adminService.getUsers();
    res.json(userList);
  }); 
  
  app.get('/api/v1/account/admin/:name/all-transactions', authenticateToken, async (req, res)=>{
